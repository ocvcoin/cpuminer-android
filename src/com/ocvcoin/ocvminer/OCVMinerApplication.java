...
public class OCVMinerApplication extends Application {
	...
	public void onCreate()
	{
		...
		//neon support disabled because it calculates the wrong hash!
		if (true || detectCPUHasNeon() == 0) {
			
			System.loadLibrary("ocv2");
			System.loadLibrary("ocvminer");
			appendToTextLog("Loading OCVMiner");
			
		} else {
			
			System.loadLibrary("ocv2-neon");
			System.loadLibrary("ocvminer-neon");			
			appendToTextLog("Loading OCVMiner with NEON support");
	...
	private void _start() {
		...
		// Register for battery status changes
		IntentFilter preferenceIntentFilter = new IntentFilter("com.ocvcoin.ocvminer.preferenceChanged");
	...
	private void startWorker() {
		if (!hasWorker()) {
			worker = new Thread(new OCVMinerRunnable(this));
			worker.start();
		}
	}
	...
	private void handleStateChange(String message) {
		Intent i = new Intent("com.ocvcoin.ocvminer.stateMessage");
		i.putExtra("com.ocvcoin.ocvminer.stateEntry", message);
	...
	protected void handleLogEntry(LogEntry entry) {
		...
		if (isWidgetActive()) {
			Intent intent = new Intent(this, OCVMinerAppWidgetProvider.class);
			intent.setAction("android.appwidget.action.APPWIDGET_UPDATE");
			int ids[] = AppWidgetManager.getInstance(this).getAppWidgetIds(new ComponentName(this, OCVMinerAppWidgetProvider.class));
			intent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_IDS,ids);
			intent.putExtra("com.ocvcoin.ocvminer.logEntry", entry);
			sendBroadcast(intent);
		}

		// Update the activity
		if (entry != null) {
			Intent i = new Intent("com.ocvcoin.ocvminer.logMessage");
			i.putExtra("com.ocvcoin.ocvminer.logEntry", entry);
			LocalBroadcastManager.getInstance(this).sendBroadcast(i);
		}
	}
